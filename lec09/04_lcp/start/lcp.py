# lcp.py - LCP (Longest Common Prefix) (START CODE)
...
def build_lcp_kasai(text, sa):
  ...
  # 힌트:
  # 1. n = len(text), lcp = [0] * n, rank = [0] * n 선언
  ...
  print("build_lcp_kasai 구현 필요")
  return [0] * len(text)  # 임시 반환값
...
def test_lcp():
  ...
  # 테스트 케이스 7: Kasai 알고리즘 테스트 (다른 문자열)
  print("=== Kasai 알고리즘 추가 테스트 ===")
  text8 = "mississippi$"
  print(f"문자열: {text8}")

  # 미리 계산된 suffix array
  suffix_array8 = [11, 10, 7, 4, 1, 0, 9, 8, 6, 3, 5, 2]
  lcp_array8 = build_lcp_kasai(text8, suffix_array8)

  print(f"Suffix Array: {suffix_array8}")
  print(f"LCP Array: {lcp_array8}")
