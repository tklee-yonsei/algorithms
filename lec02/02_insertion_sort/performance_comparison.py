...
def test_performance():
  ...
  for size in sizes:
    ...
    random_time = measure_sorting_time(insertion_sort, random_arr)
    print(f"랜덤 배열:     {random_time:.6f}초")

    # 거의 정렬된 배열
    nearly_sorted_arr = generate_nearly_sorted_array(size)
    nearly_sorted_time = measure_sorting_time(insertion_sort, nearly_sorted_arr)
    print(f"거의 정렬된:   {nearly_sorted_time:.6f}초")

    # 역순 배열
    reversed_arr = generate_reversed_array(size)
    reversed_time = measure_sorting_time(insertion_sort, reversed_arr)
    print(f"역순 배열:      {reversed_time:.6f}초")

    # 중복이 많은 배열
    few_unique_arr = generate_few_unique_array(size)
    few_unique_time = measure_sorting_time(insertion_sort, few_unique_arr)
